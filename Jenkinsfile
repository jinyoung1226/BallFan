pipeline {
	agent any
	environment {
        IMAGE_NAME = 'jinyoung1226/ballfan-back'
        DEPLOY_DIR = '/home/jinyoung/ballfan'
    }
		...
		stage('07. 기존 이미지 삭제') {
			steps {
				script {
					echo "기존 이미지 삭제"
                    sh '''
            		if [ "$(docker images -q ballfan-back)" ]; then
                		docker rmi -f $(docker images -q ballfan-back)
                		echo " ballfan-back 이미지 삭제 완료"
            		else
                		echo " ballfan-back 이미지가 존재하지 않습니다."
            		fi
            		'''
				}
			}
